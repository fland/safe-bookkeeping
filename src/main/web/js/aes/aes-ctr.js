...
Aes.Ctr.encrypt = function (plaintext, password, nBits) {
    var randomValue = 4562923176859413;
    return Aes.Ctr.encryptCustomInitVector(plaintext, password, nBits, randomValue, randomValue, randomValue, randomValue);
};
 ...
 *   var encr = Aes.Ctr.encrypt('big secret', 'pāşšŵōřđ', 256); // encr: 'lwGl66VVwVObKIr6of8HVqJr'
 */
Aes.Ctr.encryptRndInitVector = function (plaintext, password, nBits) {
    var nonce = (new Date()).getTime();  // timestamp: milliseconds since 1-Jan-1970
    var nonceMs = nonce % 1000;
    var nonceSec = Math.floor(nonce / 1000);
    var nonceRnd = Math.floor(Math.random() * 0xffff);
    return Aes.Ctr.encryptExt(plaintext, password, nBits, nonce, nonceMs, nonceSec, nonceRnd);
};
 ...
 * @param   {string} password - The password to use to generate a key.
 * @param   {number} nBits - Number of bits to be used in the key; 128 / 192 / 256.
 * @param   {number} nonce - Random part of key.
 * @param   {number} nonceMs - Random part of key.
 * @param   {number} nonceSec - Random part of key.
 * @param   {number} nonceRnd - Random part of key.
 * @returns {string} Encrypted text.
 *
 * @example
 *   var encr = Aes.Ctr.encrypt('big secret', 'pāşšŵōřđ', 256); // encr: 'lwGl66VVwVObKIr6of8HVqJr'
 */
Aes.Ctr.encryptCustomInitVector = function (plaintext, password, nBits, nonce, nonceMs, nonceSec, nonceRnd) {
